void l23_two_conv(
  float inp0[512][4][4],
  float out0[512][4][4],
  float weight0[512][512][3][3],

  float inp1[256][8][8],
  float out1[512][4][4],
  float weight1[512][256][1][1]
) {	// L1193

  float padding[512][6][6];	// L1194
      ...
      for (int h = 0; h < 6; h++) {        // L6
        for (int w = 0; w < 6; w++) {      // L7
          ...
          if (h >= 1 && h < 5 && w >= 1 && w < 5) {    // L35
            float v11 = inp0[b][c][(h - 1)][(w - 1)];    // L36
            v = v11;    // L37
          }
          float v12 = v;       // L39
  ...
  float S_b_co_ho_wo_0_reuse_2[8][3][6];     // L7
  #pragma HLS array_partition variable=S_b_co_ho_wo_0_reuse_2 complete dim=1

  float S_b_co_ho_wo_0_reuse_3[8][3][3];     // L8
        ...
        for (int wo = 0; wo < 6; wo++) {  // L12
          ...
          for (int v9o = 0; v9o < 64; v9o++) {
            ...
            for (int v9 = 0; v9 < 8; v9++) {    // L13
              float v10 = S_b_co_ho_wo_0_reuse_2[v9][1][wo];     // L14
              S_b_co_ho_wo_0_reuse_2[v9][0][wo] = v10;    // L15
              float v11 = S_b_co_ho_wo_0_reuse_2[v9][2][wo];     // L16
              S_b_co_ho_wo_0_reuse_2[v9][1][wo] = v11;    // L17
              float v12 = padding[b][v9][ho][wo];     // L18
              S_b_co_ho_wo_0_reuse_2[v9][2][wo] = v12;    // L19
            }
            if ((ho - 2) >= 0) {  // L21
              for (int v13 = 0; v13 < 8; v13++) {       // L22
                for (int v14 = 0; v14 < 3; v14++) {       // L23
                  float v15 = S_b_co_ho_wo_0_reuse_3[v13][v14][1];       // L24
                  S_b_co_ho_wo_0_reuse_3[v13][v14][0] = v15;      // L25
                  float v16 = S_b_co_ho_wo_0_reuse_3[v13][v14][2];       // L26
                  S_b_co_ho_wo_0_reuse_3[v13][v14][1] = v16;      // L27
                  float v17 = S_b_co_ho_wo_0_reuse_2[v13][v14][wo];      // L28
                ...
                for (int ci = 0; ci < 8; ci++) {       // L37
                  for (int r = 0; r < 3; r++) {      // L38
                    for (int c = 0; c < 3; c++) {    // L39
                      float v22 = S_b_co_ho_wo_0_reuse_3[ci][r][c];      // L40
                      float v23 = weight0[co][ci][r][c];      // L41
                      int16_t v24 = v22;  // L42
                      int16_t v25 = v23;  // L43
                      int16_t v26 = v24 * v25;    // L44
                      float v27 = v26;   // L45
                      float v28 = v;     // L46
                      float v29 = v28 + v27;     // L47
                      v = v29;    // L48
                    }
                  }
                }
          ...
          for (int ci = 0; ci < 256; ci++) {   // L12
            for (int r = 0; r < 1; r++) {  // L13
              for (int c = 0; c < 1; c++) {        // L14
                float v11 = inp1[b][ci][((ho * 2) + r)][((wo * 2) + c)]; // L15
                float v12 = weight1[co][ci][r][c];  // L16
                int16_t v13 = v11;      // L17
                int16_t v14 = v12;      // L18
