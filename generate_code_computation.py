...
class GenerateCodeComputation:
    ...
    def write_load(self, arr, original_name, size, size_split, size_burst, burst_size, it_dim):
        lines = []
        tab = ""
        arg = []
        split_array = True
        ...
        size_split_str = list(map(str, size_split))
        arg += [f"{self.data_type} {original_name}[{']['.join(size_split_str)}]"]
        ...
        for k in range(nb_loop):
            ...
            if split_array:
                lines += [f"{tab}   int i{k}_0;\n"]
                lines += [f"{tab}   int i{k}_1;\n"]
        ...
        for bb in range(burst_size):
            ...
            for k in range(ub):
                ...
                if k%2==0:
                    id_it += 1
                if split_array:
                    split = size_split[2*id_it+1]
                if split_array and dd:
                    post = "_0"
                elif split_array and not dd:
                    post = "_1"
                else:
                    post = ""
                if not split_array and k == nb_loop - 1:
                    str_ += f"[i{id_it}{post} + {bb}]"
                else:
                    if k%2==0 and split_array:
                        inc = 0
                        if k == 2*nb_loop - 2:
                            inc = bb
                        lines += [f"{tab}i{id_it}_0 = (i{id_it}+{inc}) / {split};\n"]
                        lines += [f"{tab}i{id_it}_1 = (i{id_it}+{inc}) % {split};\n"]
                    str_ += f"[i{id_it}{post}]"
    ...
    def write_store(self, arr, original_name, size, size_split, size_burst, burst_size, it_dim):
        lines = []
        tab = ""
        arg = []
        split_array = True
    ...
    def generate_code(self):
        ...
        for arr in arrays:
            ...
            for line in self.lines_nlp_file:
                if "#Size" in line and "array" in line and f"{arr}:" in line:
                    ...
                    self.original_size[arr] = size
                    self.original_size_split[arr] = size

                    self.split_size[arr] = size
                        ...
                        for y, pos in enumerate(possi):
                            ...
                            for l in range(1, len(self.schedule[stat]), 2):
                                ...
                                for dim in list(self.which_loop_iterate_array_dim[arr].keys()):
                                    if curr_loop in self.which_loop_iterate_array_dim[arr][dim]:
                                        ...
                                        if stat not in self.partial_transfer:
                                            self.partial_transfer[stat] = {}
                                        if "_S" in arr:
                                            arr2 = arr.split("_S")[0]
                                        else:
                                            arr2 = arr
                                        if arr2 not in self.partial_transfer[stat]:
                                            self.partial_transfer[stat][arr2] = []
                                        self.partial_transfer[stat][arr2] += [self.iterators[self.schedule[stat][l]]]
                            ...
                            for g in range(stat):
                                ...
                                if f"{self.data_type} {arr}_S{g}[{']['.join(size_)}];\n" in lines:
                                    ...
                                    deja_vu = True
                            size_ = list(map(str, size_))
                            self.original_size[arr] = size_
                            self.original_size_split[arr] = size_

                            new_size = [1 for k in range(2*len(size_))]


                            new_size = [1 for k in range(2*len(size_))]
                            for k in range(len(size_)):
                                new_size[2*k] = size_[k]
                            ...
                            for key in list(self.log.keys()):
                                if "AP" in key:
                                    ...
                                    dim = dd[-1]
                                    var = "_".join(dd[1:-1])
                                    factor = max(int(self.log[key]), 1)

                            new_size = list(map(str, new_size))
                            for k in range(len(size)):
                                new_size[2*k] = size_[k]
                                possible_loops = self.which_loop_iterate_array_dim[arr][k]
                                curr_loop = 0
                                for pos_l in possible_loops:
                                    if pos_l in self.schedule[stat][1::2]:
                                        curr_loop = pos_l
                                        break

                                curr_loop = self.which_loop_iterate_array_dim[arr][k][0]
                                if self.original_size[arr][k] == size[k] and self.original_size[arr][k] == size_[k]:
                                    new_size[2*k+1] = self.log[f"TC{curr_loop}_2"]
                                    new_size[2*k] = int(size_[k]) // int(self.log[f"TC{curr_loop}_2"])
                            new_size = list(map(str, new_size))
                            is_write = False
                            if arr in self.statements[stat].split("=")[0]:
                                is_write = True
                            if deja_vu:
                                input_for_statement[stat] += [f"{self.data_type} {arr}_S{gg}[{']['.join(new_size)}];\n"]
                                if is_write:
                                    output_for_statement[stat] += [f"{self.data_type} {arr}_S{gg}[{']['.join(new_size)}];\n"]
                            else:

                                lines += [f"    {self.data_type} {arr}_S{stat}[{']['.join(new_size)}];\n"]
                                arrays_on_chip += [f"{arr}_S{stat}"]
                                input_for_statement[stat] += [f"{self.data_type} {arr}_S{stat}[{']['.join(new_size)}];\n"]
                                if f"{arr}_S{stat}" not in list(self.size_on_chip.keys()):
                                    self.size_on_chip[f"{arr}_S{stat}"] = {}
                                for kkk2, ddd2 in enumerate(new_size):
                                ...
                                if is_write:
                                    ...
                                    output_for_statement[stat] += [f"{self.data_type} {arr}_S{stat}[{']['.join(new_size)}];\n"]
                                

                            self.split_size[f"{arr}_S{stat}"] = new_size
                            self.split_size[f"{arr}"] = new_size
                            new_size2 = [1 for k in range(len(size))]
                            for k in range(len(size)):
                                new_size2[k] *= int(new_size[2*k])
                                new_size2[k] *= int(new_size[2*k+1])
                            self.original_size_split[f"{arr}_S{stat}"] = new_size2
            if tot_trans:
                new_size = [1 for k in range(2*len(size))]
                for k in range(len(size)):
                    new_size[2*k] = size[k]
                ...
                for key in list(self.log.keys()):
                    if "AP" in key:
                        ...
                        if var in arr:
                            new_size[2*int(dim)+1] = max(int(factor),1)
                            new_size[2*int(dim)] = int(size[int(dim)]) // int(factor)
                new_size = list(map(str, new_size))

                self.original_size[arr] = size
                self.original_size_split[arr] = size
                self.split_size[arr] = new_size
                lines += [f"    {self.data_type} {arr}[{']['.join(new_size)}];\n"]
                    ...
                    for j in range(1, len(self.schedule[k]), 2):
                        for ll in self.lines_nlp_file:
                            ...
                            if f"#loop_{self.schedule[k][j]} iterates Array {arr}" in ll:
                                if f"{self.data_type} {arr}[{']['.join(new_size)}];\n" not in input_for_statement[k]:
                                    input_for_statement[k] += [f"{self.data_type} {arr}[{']['.join(new_size)}];\n"]
                                    is_write = False
                                    if arr in self.statements[k].split("=")[0]:
                                        is_write = True
                                    if is_write:
                                        output_for_statement[k] += [f"{self.data_type} {arr}[{']['.join(new_size)}];\n"]
        lines += ["\n"]
        ...
        for key in list(self.log.keys()):
            if "AP" in key:
                ...
                for arr2 in arrays_on_chip:
                    if var == arr2:
                        lines += [f"#pragma HLS ARRAY_PARTITION variable={var} cyclic factor={factor} dim={2*int(dim)+2}\n"]
                    elif var in arr2:

                        lines += [f"#pragma HLS ARRAY_PARTITION variable={arr2} cyclic factor={factor} dim={2*int(dim)+2}\n"]
        ...
        for key in list(input_for_statement.keys()):
            for elemt in input_for_statement[key]:
                ...
                tmp_size = [1 for k in range(len(self.size_array_per_dim[original_name]))]
                for k in range(len(size)//2):
                    tmp_size[k] *= size[2*k]
                    tmp_size[k] *= size[2*k+1]

                
                if len(tmp_size) == len(list(self.size_array_per_dim[original_name])):
                    for id_, ss in enumerate(tmp_size):
                ...
                ori_name = name
                if name not in list(self.original_size.keys()):
                    ori_name = name.split("_S")[0]
                
                ll = self.write_load(name, original_name, self.original_size_split[name], self.split_size[name] , buffer_size, burst_size_, it_dim)
                if ll not in deja_vu:
                    load_lines += ll
                    deja_vu += [ll]
                if elemt in output_for_statement[key]:
                    ll = self.write_store(name, original_name, self.original_size_split[name], self.split_size[name], buffer_size, burst_size_, it_dim)
                    if ll not in deja_vu:
                        write_lines += ll
                        deja_vu += [ll]
        ...
        for k in range(len(tasks)):
            ...
            for j, line in enumerate(new_task):
                if "for" in line:
                    it = line.split("int ")[1].split(" =")[0]
                    lb = line.split("int ")[1].split(" =")[1].split(";")[0].replace(" ", "")
                    ub = line.split(";")[1].split("<")[1].replace(" ", "")
                    for_ += [(it, lb, ub)]
                    dic_for[it] = (lb, ub)
                if "void" in line:
                    arr_tmp = line.split("(")[1].split(")")[0].split(", ")
                if "=" in line and "for" not in line:
                    it = line.split("=")[0].replace(" ", "")
                    other = line.split("=")[1].replace(" ", "").replace(";", "").replace("\n", "")
                    dic_decl[it] = other
                if "int" in line and "for" not in line:
                    it = line.replace(";", "").replace("\n", "").replace("int", "").replace(" ", "")
                    new_task[j] = f"    int {it}, {it}_0, {it}_1;\n"
                    line_to_declare = j
            ...
            for arr2 in arr_tmp:
                ...
                if "[" in arr2:
                    index = arr2.index("[")
                    size_ = arr2[index+1:-1].split("][")
                    arr_ += [(name, size_)]
                    dic_arr[name] = size_
            stat = ""
            for j, line in enumerate(new_task):
                if "[" in line and "=" in line and "for" not in line:
                    stat = line
                    line_stat = j
                    break
            if stat != "":
                ...
                inside_bracket = False
                curr_array = ""
                for id_, elemt in enumerate(stat):
                    if elemt == "[":
                        inside_bracket = True
                    if elemt == "]":
                        inside_bracket = False
                ...
                op += [";\n"]
                nb_level = -1
                for id_arr, curr_arr in enumerate(larr):
                    name = curr_arr.split("[")[0]
                    original_it = curr_arr.replace(name, "")
                    new_decl += f"{name}"
                    if "[" in curr_arr:
                        name = curr_arr.split("[")[0]
                        index = curr_arr.index("[")
                        its = curr_arr[index+1:-1].split("][")
                        for id_dim, it in enumerate(its):
                            post_it = ""
                            if it[-1] == "2":
                                nb_level = 3-it[:-1].count("+")
                                it = it[:-1].split("+")[-1]# FIXME
                                post_it = "2"

                            if "+" in it:
                                continue
                            if int(dic_for[f'{it}2'][1]) == int(dic_arr[name][2*id_dim+1]):
                                dd = 0
                                if f"{it}_{name}_{dd}" not in new_it:
                                    l01 = dic_decl[it].replace(f"{it}2", "0")
                                    if nb_level == 2:
                                        if k in self.partial_transfer and name in self.partial_transfer[k]:
                                            if it in self.partial_transfer[k][name]:
                                                l01 = l01.replace(f"{it}0", "0")
                                    l01 = f"({l01})/{dic_arr[name][2*id_dim+1]}"
                                    new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                    new_it += f"{it}_{name}_{dd} = {l01};\n"
                                new_decl += f"[{it}_{name}_{dd}]"
                                dd = 1
                                if f"{it}_{name}_{dd}" not in new_it:
                                    new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                    new_it += f"{it}_{name}_{dd} = {it}2;\n"
                                new_decl += f"[{it}_{name}_{dd}]"
                            else:
                                if int(dic_arr[name][2*id_dim]) == 1:
                                    dd = 0
                                    if f"{it}_{name}_{dd}" not in new_it:
                                        new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                        new_it += f"{it}_{name}_{dd} = 0;\n"
                                    new_decl += f"[{it}_{name}_{dd}]"
                                    dd = 1
                                    if f"{it}_{name}_{dd}" not in new_it:
                                        new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                        new_it += f"{it}_{name}_{dd} = {it}{post_it};\n"
                                    new_decl += f"[{it}_{name}_{dd}]"
                                else:
                                    dd = 0
                                    if f"{it}_{name}_{dd}" not in new_it:
                                        new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                        new_it += f"{it}_{name}_{dd} = {it}{post_it} / {dic_arr[name][2*id_dim+1]};\n"
                                    new_decl += f"[{it}_{name}_{dd}]"
                                    dd = 1
                                    if f"{it}_{name}_{dd}" not in new_it:
                                        new_task[line_to_declare] += f"    int {it}_{name}_{dd};\n"
                                        new_it += f"{it}_{name}_{dd} = {it}{post_it} % {dic_arr[name][2*id_dim+1]};\n"
                                    new_decl += f"[{it}_{name}_{dd}]"
                    if op[id_arr] == "=" and original_op != "":
                        new_decl += f" {original_op} "
                    else:
                        new_decl += op[id_arr]
                        

            new_task[line_stat] = f""
            new_task[line_stat] += new_it  
            new_task[line_stat] += new_decl 
