...
# ISCC_CMD = "/opt/polyopt/default/sources/polyopt-hls/pocc/math/install-barvinok/bin/iscc"
ISCC_CMD = "iscc"


class ISCC:

    def __init__(self, data_type, folder, fully_distributed_file, schedule, dic, operations, arrays_size, chip, dep, operation_list):
    ...
    def run_iscc(self):
        ...
        for pos in all_pos:
            ...
            if RaW and WaW and WaR:
                # print(f"ISCC is correct with schedule {schedule}")


                analysis_ = analysis.Analysis(self.data_type, schedule, dic, self.operations, self.arrays_size, self.chip, self.dep, self.operation_list)
                UB = analysis_.UB
                LB = analysis_.LB
                ...
                code_generation_ = code_generation.CodeGeneration(self.data_type,analysis_, schedule, {}, UB, LB, statements, iterators, self.fully_distributed_file, headers, arguments, name_function, pragmas, pragmas_top, optimize_burst, False, True)
                return

        # print(f"ISCC is correct with original schedule {schedule_ori}")


        dic = copy.deepcopy(self.dic)
        analysis_ = analysis.Analysis(self.data_type, schedule_ori, dic, self.operations, self.arrays_size, self.dep, self.operation_list)
        UB = analysis_.UB
        LB = analysis_.LB
        operations = self.operations
        statements = analysis_.statements
        ...
        code_generation_ = code_generation.CodeGeneration(self.data_type,analysis_, schedule, {}, UB, LB, statements, iterators, self.fully_distributed_file, headers, arguments, name_function, pragmas, pragmas_top, optimize_burst, True)