    ...
    def create_nlp(self):
        ...
        for k in range(len(self.schedule)):
            ...
            if len(first_time_see) > 0:
                ...
                con_read2.append(" + ".join(first_time_see))
            ...
            for loop in self.schedule[k][1::2]:
                ...
                for array in arrays:
                    ...
                    tcc = []
                    tcc_iterated = []
                    for schedd in self.schedule[k][1::2]:
                        iterate_array = False
                    ...
                    if f"transfer_{array}_S{k}_under_loop{loop} binary;" in var:
                        read_level.append(f"transfer_{array}_S{k}_under_loop{loop} * {tcc} * {tcc_iterated} * footprint_{array}_S{k} / {tcc_iterated} / burst_{array}_S{k}_under_loop{loop}")
                        if array_information[array]["schedule"][-1] < k or array == write_array:
                            write_level.append(f"transfer_{array}_S{k}_under_loop{loop} * {tcc} * footprint_{array}_S{k}  / burst_{array}_S{k}_under_loop{loop}")
