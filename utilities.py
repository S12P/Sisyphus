...
def process_nlp_results(schedule, nlp_file, nlp_log):
    ...
    for line in log_lines:
        if "objective" in line or "time limit" in line:
            objective_seen = True
        if objective_seen and "=" in line and "absmipgap" not in line:
...
def run_vitis_hls(tcl_file, path, timeout):
    """
    Run the Vitis HLS command with the specified TCL file in the given path, 
    and apply a timeout to the execution.
    ...
    try:
        ...
        print(e.stdout)  # Print any partial output captured before the timeout
        print("Please consider increasing the timeout value or optimizing the command.")
        return 0
...
def print_summary(folder, source_file):
    report_file = f"{folder}/kernel_nlp/solution/syn/report/kernel_nlp_csynth.rpt"
    try:
